# Import libraries
#============================================================================================================================================
rho=0.4 # change desired rho value here
dir <- '/Users/harleymount/Desktop/CBU_analysis_recent_for_alex'
setwd('/Users/harleymount/Ensminger_BFG/')
#============================================================================================================================================
...
prs.file <- getArg("PRS","res/LP_prs.tsv")
# The barcode descriptor file.
barcodes.file <- getArg("barcodeDescriptor","res/barcodes_V2.csv")
# working directory
...
#png(paste(dir,paste0("ips_barplot_",rho,".png"),sep=""),1200,500)
pdf(file=paste0(dir,'ips_barplot_',rho,'.pdf'),width = 20, height = 10)
...
op <- par(mar=c(0,4,0,1)+.1,cex=.7,las=2) # changed to ..5 here 
plot(0,type="n",axes=FALSE,xlab="",ylab="",ylim=c(0,3),xlim=c(0,n.ints))
axis(2,at=1:3-.5,labels=c("Union","HS PRS","LP PRS"),tick=FALSE)
...
op <- par(mar=c(0,4,0,1)+.1,cex=.4) # changed cex here from .7 to .4  #THIS IS X AXIS TEXT SIZE 
...
best.ips.matrix_log<-best.ips.matrix+abs(min(best.ips.matrix))+200
...
prs_matrix<-best.ips.matrix[1:34,1:34]
logger$info("Drawing IPS heatmap")
yp <- new.yogi.plotter()
...
#----log PRS matrix
prs_matrix<-best.ips.matrix[1:34,1:34]
prs_matrix<-prs_matrix+abs(min(prs_matrix))+200
prs_matrix<-log2(prs_matrix)
logger$info("Drawing IPS heatmap")
yp <- new.yogi.plotter()

postscript(paste(dir,"ips_heatmap_log2PRS_", rho,".ps",sep=""), width=20, height=20)
png(paste(dir,"ips_heatmap_PRS_", rho,".png",sep=""),1000,1000)
ad.names <- sapply(rownames(prs_matrix),function(name) with(barcodes,unique(symbol[id==name])))
db.names <- sapply(colnames(prs_matrix),function(name) with(barcodes,unique(symbol[id==name])))
ad.prs.idx <- max(which(sapply(rownames(prs_matrix),function(name) with(barcodes,unique(type[id==name])))=="PRS"))
