...
#mat<-best.ips.matrix
#t<-mccs$union[best.param,"best.t"]
...
new.cytoscape <- function() {
	...
	#i<-1
		for (i in 1:length(unique_list)){
		  ...
		  if(!is.na(subset_df_2[1,1])){colnames(subset_df_2)<-c("n2", "n2.name", "n1", "n1.name", "weight", 'width')} #re-order DB data, so that we can just combine everything without directionality
		...
		#undirectional_df<-undirectional_df[!duplicated(undirectional_df$code),]#not removing thhe duplicate so I can show PPI captured in two directions
	...
	from.matrix <- function(mat, t, out.file="network.xgmml", 
		...
		apply(edges,1, function(edge) {
		  x<-0
			label <- paste(edge["n1"],"(DirectedEdge)",edge["n2"],x)
			x<-x+1
