...
def calculate_freq(plus_his, minus_his):
    ...
    #heat_freq(med_freq)
    return plus_his_freq, minus_his_freq, row_freq, col_freq, AD_NAMES, DB_NAMES
...
def get_mcc(dicts, gold_st, plus_his_freq, row_cut, col_cut):
...
#========================Custom cde here         
        ...
        plus=plus_his_freq.unstack().reset_index()
        plus.columns = ['DB_name', 'AD_name', 'plus_freq']
        
        
        
        #this merging fails, need to amek a new column like AD_EV-BC1_DB_EV-BC1
        i['barcode']=i.AD_name+i.DB_name
        plus['barcode']=plus.AD_name+plus.DB_name
        
        i = pd.merge(i, plus,on='barcode')############################CONTINUE WORKING HERE< NOT DONE THIS LINE 
        #thus far code above makes table similar to what is made, now apply cutoff boolean
        

        i["cut"] = i.plus_freq > row_cut*col_cut
        i["screen"] = (i.is_AD & i.is_DB & i.cut).astype(int)
...
def score_main(plus_his, minus_his, weights, floor_perc, gold_st): #I will make a new score_main function that just uses one sample instead of 3
    ...
    plus_his_freq, minus_his_freq, row_freq, col_freq, AD_NAMES, DB_NAMES = calculate_freq(plus_his, minus_his)#for this line I will need to change it to only do teh calculations for my one combined counts sample
    ...
    row_sorted = sorted(row_freq.tolist()) #these values are used to determine cutoffs
    col_sorted = sorted(col_freq.tolist())#these values are used to determine cutoffs --> need to figure out how to do this with the non marginal plus his matrix
...
#==================I added this here
        #MY NEW STRATEGY IS TO STILL FIND THE AD AND DB FREQ CUTOFF LIKE IN OLD CODE, THEN MULTIPLY THE TWO TOGETHER AND MAKE THIS THE FLOOR 
        combined_freq_cutoff= row_cut*col_cut
        plus_his_freq=plus_his_freq.where(plus_his_freq > combined_freq_cutoff, combined_freq_cutoff)   

        # rebuild matrix from two vectors
        #freq_mx = plus_his_freq
    ...
    #--------------------------------------
        ...
        IS_normed = get_norm_score(weight, minus_his_freq, plus_his_freq)
        ...
        #--------------Need to fix the get MCC so it doesnt depend on AD and DB freq 
        
        
        
        mcc_list = get_mcc(dicts, gold_st, plus_his_freq, row_cut, col_cut)
        mcc_list["weight"] = weight
        mcc_list["floor"] = floor
        output_csv = output_csv.append(mcc_list)
        #break 
    return output_csv, row_freq, col_freq, minus_his_freq, plus_his_freq, AD_NAMES, DB_NAMES
...
def dk_main(litbm, max_weight, max_rank, max_floor, minus_his_freq, row_freq, col_freq, AD_NAMES, DB_NAMES, fname, plus_his_freq):
...
#------------------adding custom code here to not use marginal freq like score_main
...
#    DB_freq = DB_freq.to_frame()
    ...
    IS_normed = get_norm_score(max_weight, minus_his_freq, plus_his_freq)
...
#=================HM custom code ends
    ...
    mcc = get_mcc(dicts, litbm, plus_his_freq, row_cut, col_cut)
...
def main(plus_his, minus_his, gold_st):
    ...
    plus_his_freq = freq_NOZ(plus_his)
...
#-----DK analkysis method
output_csv, row_freq, col_freq, minus_his_freq, plus_his_freq, AD_NAMES, DB_NAMES = score_main(plus_his, minus_his, param.weights, param.floor_perc, yi)
...
dk_main(litbm, max_weight, max_rank, max_floor, minus_his_freq, row_freq, col_freq, AD_NAMES, DB_NAMES, "dk_mcc_summary_litbm_full_ranks.csv", plus_his_freq)
