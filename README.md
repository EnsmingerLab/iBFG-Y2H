...
<p>The *_combined_counts.csv files are used for scoring. Obviously need a combined counts for the selective (-HIS) and non-selective condition (+HIS). I do the scoring of these combined counts locally in python. Download combined counts.csv files for both samples to a directory that will be used for scoring. Using spyder I then manually run each line of the score_HM.py or noz_score_HM.py to analyze the counts files.</p>
