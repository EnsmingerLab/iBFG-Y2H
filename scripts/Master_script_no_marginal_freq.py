...
#---------------------figuring out how to subset IS normed with top rank 
g.nth(1).dropna(how='any')

g.groups
groups=g.describe()

g.get_group('PSMD2_SH3GLB1')
...
g_all=transform.sort_values(["Score"], ascending=False).groupby(['Interaction'])

g_all.nth(1).dropna(how='any').sort_values(["Score"], ascending=False).to_csv("DK_best_IS_rank_scores.csv", index=False)#line to get the best ranked interaction score matrix based on MCC by changing .nth(x)

#once this is applid to teh gold standard to choose the best rank for capturing PPI, we can then apply it to the entire set of PPIs to find the interaction score list 
...
def score_main(plus_his, minus_his, weights, floor_perc, gold_st): #I will make a new score_main function that just uses one sample instead of 3
    ...
    all_pairs = list(itertools.product(AD_intersect, DB_intersect)) #make 1056 different combinations of AD and DB GOLD PPI
...
mcc_sum="DK_mcc_summary_yi1.csv"
